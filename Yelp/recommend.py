...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min(centroids, key=lambda y: distance(location, y))
    # END Question 3
...
def find_centroid(cluster):
    ...
    long_lst=[]
    for r in cluster:
        lat_lst.append(restaurant_location(r)[0])
        long_lst.append(restaurant_location(r)[1])
    avg_lat=mean(lat_lst)
    avg_long=mean(long_lst)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor=[find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]
    return max(predictor, key=lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            d[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            d[restaurant_name(restaurant)] = predictor(restaurant)
    return d
